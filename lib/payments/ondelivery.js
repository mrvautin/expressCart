...
router.post('/checkout_action', async (req, res, next) => {
    ...
    const paymentConfig = getPaymentConfig('ondelivery');

    const orderDoc = {
        orderPaymentId: getId(),
        orderPaymentGateway: 'Ondelivery',
        orderPaymentMessage: 'Your payment was successfully completed',
        orderPaymentInstruction : `Please pay ${req.session.totalCartNetAmount} ${config.currencyISO} on delivery`,
