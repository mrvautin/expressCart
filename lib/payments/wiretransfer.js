...
router.post('/checkout_action', async (req, res, next) => {
    ...
    const paymentConfig = getPaymentConfig('wiretransfer');

    const orderDoc = {
        orderPaymentId: getId(),
        orderPaymentGateway: 'Wiretransfer',
        orderPaymentMessage: 'Wiretransfer pending',
        orderPaymentInstruction : `Please pay ${req.session.totalCartNetAmount} ${config.currencyISO} On account ${paymentConfig.accountNumber} within 3 days.`,
    ...
    try{
        ...
        .then(() => {
            ...
            req.session.message = 'Your order was successfully placed. Payment for your order will be done with wire transfer';
            ...
            <p><strong>Please pay {orderDoc.orderTotal} On account "XXXXXXXXXXXXXXXX" within 3 days.</strong></p>;
            <p><strong>Transaction ID: </strong>${orderDoc.orderPaymentId}</p>`;
